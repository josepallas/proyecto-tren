...
public class TrainServiceImpl implements TrainService {
	...
	public List<CarType> findClassTypes(Long stopId) throws InstanceNotFoundException {
		...
		return carDao.findClassByTrain(route.getTrain().getTrainId());
	}
	...
	public List<TravelInfo> findTravels(Calendar day, String origin,
		...
		List<Route> routes = routeDao
				.findRoutesByDay(getWeekDay(day), routesId);
		List<TravelInfo> travels = new ArrayList<TravelInfo>();
		for (Route r : routes) {
			int emptySeats=carDao.getCapacityByTrain(r.getTrain().getTrainId())-ticketDao.getNumberOccupiedSeats(day, r.getRouteId());
			TravelInfo travelInfo = new TravelInfo(r.getRouteId(),
					r.getRouteName(), r.getRouteDescription(), r.getTrain(),
					r.getPrice(),emptySeats);
	...
	public Station createStation(String stationName, String city, String address)
		...
		try {
			...
			throw new DuplicateInstanceException(stationName,
