...
@Repository("carDao")
public class CarDaoHibernate extends GenericDaoHibernate<Car, Long> implements
		CarDao {
	...
	public List<Car> findCarsByTrain(CarType carType, Long trainId) {
		...
		List<Car> result = getSession()
				.createQuery(
						"SELECT c FROM Car c  WHERE c.train.trainId= :trainId"
								+ " AND c.carType= :carType")
				.setParameter("carType", carType)
				.setParameter("trainId", trainId).list();
		return result;
	}
	...
	public List<CarType> findClassByTrain(Long trainId) {
		return getSession()
				.createQuery(
						"SELECT distinct c.carType FROM Car c "
								+ "WHERE c.train.trainId= :trainId")
				.setParameter("trainId", trainId).list();
	}
	public int getCapacityByTrain(Long trainId) {
		long capacity =(Long)getSession()
				.createQuery(
						"SELECT SUM(capacity) FROM Car c WHERE c.train.trainId = :trainId")
